    ...
    private fun calculateUnits(){
        if(currentVisibleView== METRIC_UNITS_VIEW){
            if(validateMetricUnits()){
                ...
                Toast.makeText(this@BMI_Activity,"Please enter valid value",Toast.LENGTH_SHORT).show()
            }
        }else{
            if(validateUsUnits()){
                ...
                val heightValue=usUnitHeightValueInch.toFloat() + (usUnitHeightValueFeet.toFloat()*12.0f)

                val bmi=703*(usUnitWeightValue/(heightValue*heightValue))
//                var roundedBMIResult = BigDecimal(bmi.toDouble()).setScale(2, RoundingMode.HALF_EVEN).toString()
//                val bmiResultRounded = roundedBMIResult.toFloat()
